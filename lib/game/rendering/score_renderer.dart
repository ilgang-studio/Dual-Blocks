...
void _drawHeader({
  ...
  final scale = (layout.scoreRect.height / 72).clamp(0.85, 1.3);
  final headerTopY = layout.scoreRect.top + (8 * scale);
  final topLeftX = layout.scoreRect.left + (16 * scale);
  ...
  final scoreFontSize = ((24.0 * scale).clamp(22.0, 32.0)) + (pulse * 4.0);
  ...
  final topLabelPainter = TextPainter(
    text: TextSpan(
      ...
      style: TextStyle(
        ...
        fontSize: (13.0 * scale).clamp(11.0, 16.0),
  ...
  topLabelPainter.paint(canvas, Offset(topLeftX, headerTopY));
  ...
  final centerTitlePainter = TextPainter(
    text: TextSpan(
      ...
      style: TextStyle(
        ...
        fontSize: (11.0 * scale).clamp(10.0, 14.0),
  ...
  final metaY = layout.scoreRect.bottom - metaPainter.height - (4 * scale);
  metaPainter.paint(
    canvas,
    Offset(layout.scoreRect.center.dx - (metaPainter.width / 2), metaY),
  );
  ...
  final comboPainter = TextPainter(
    text: TextSpan(
      ...
      style: TextStyle(
        ...
        fontSize: (10.0 * scale).clamp(9.0, 12.0),
        fontWeight: FontWeight.w800,
        letterSpacing: 1.0,
        shadows: [
          Shadow(
  ...
  if (isComboActive) {
    comboPainter.paint(
      ...
      Offset(
        ...
        metaY - comboPainter.height - (2 * scale),
      ),
    );
  }
...
void _drawHeaderStackDots({
  ...
  final spacing = 19.0 * (layout.scoreRect.height / 72).clamp(0.9, 1.2);
  final baseY =
      layout.scoreRect.bottom -
      (14 * (layout.scoreRect.height / 72)).clamp(10.0, 20.0);
  final startX = isAngel
      ? layout.scoreRect.left + (34 * (layout.scoreRect.height / 72))
      : layout.scoreRect.right -
            (34 * (layout.scoreRect.height / 72)) -
            (spacing * 2);
